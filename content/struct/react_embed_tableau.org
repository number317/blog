#+title: React Embed Tableau
#+summary: 在 react 中嵌入 tableau server 的图表
#+url: /struct/react_embed_tableau.html
#+author: cheon
#+email: cheon0112358d@gmail.com
#+date: 2020-11-22T20:03:34+08:00
#+publishdate: 2021-04-12T20:03:34+08:00
#+tags: react tableau
#+categories: struct
#+draft: false
...
** 前置工作
想要将 tableau server 的图表嵌入网页中，首先要解决的是身份认证的问题。工作原理可以参考[[https://help.tableau.com/current/server/zh-cn/trusted_auth.htm][官方文档]]。
从文档中可以看出，要解决以下几个问题：
...
** 嵌入代码
*** 引入 js
首先需要引入 tableau server 的 js 文件。这里用的是 2019.1 的 tableau server 版本。然而根据[[https://help.tableau.com/v2019.1/api/js_api/en-us/JavaScriptAPI/js_api_concepts_get_API.htm][官方文档]]上记载的 js 版本引入之后发现用不了，最后测试出来应该引用 tableau-2.2.2.min.js 文件。
...
** 疑难杂症
- firefox 刷新报错
[[./images/react_embed_tableau_1.jpg]]
